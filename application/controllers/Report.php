		...
		public function toFile(string $sql) : void {
			...
			if (is_array($sqlData) AND !empty($sqlData)) {
				foreach ($sqlData as $temp) {
					...
					array_push($carHistoryOut, $carHistory);
				}
				array_unshift($carHistoryOut, array_unique($fieldNames));
			}

			$xlsx = self::fromArray($carHistoryOut)
				->setDefaultFont('Arial')
				->setDefaultFontSize(12);
			$xlsx->downloadAs('Отчет_'.date('d-m-Y_H-i-s', time()).'.xlsx');
		}
